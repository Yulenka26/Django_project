...
class PostViewTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="test_user", password="123456789Dj"
        )
    ...
    def test_post_detail_404(self):
        response = self.client.get(
            reverse("blog:post_detail", args=["Page-does-not-exist"])
        )
...
class CategoryViewTest(TestCase):
    def setUp(self):
        ...
        self.cat_3 = Category.objects.create(
            title="test_category3", slug="test-category3"
        )
        self.cat_4 = Category.objects.create(
            title="test_category4", slug="test-category4"
        )
